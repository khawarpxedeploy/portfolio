...
class Mollie {
    ...
    public static function fallback($request_from)
    {
        if ($request_from == 'customer' || $request_from == 'api') {
            return url('customer/payment/mollie'); 
        }elseif ($request_from == 'user') {
    ...
    public static function make_payment($array){
        ...
        try {
            ...
            $payment = $mollie->payments->create([
                ...
                "redirectUrl" => Mollie::fallback($request_from),
            ]);
            ...
            return redirect(Mollie::redirect_if_payment_faild($request_from));  
        }
    ...
    public function status(Request $request){
        ...
        if(Session::has('pay_id') && Session::has('mollie_credentials')){
              ...
              if ($payment->isPaid())
              {
                ...
                return redirect(Mollie::redirect_if_payment_success($request_from));
              }
              Session::forget('pay_id');
              Session::forget('mollie_credentials');
              return redirect(Mollie::redirect_if_payment_faild($request_from));  
    ...
    public static function isfraud($cred){
        ...
        try {
            ...
            return $payment->isPaid() === true ? 1 : 0;
        } catch (\Throwable $th) {
            return 0;
        }
