...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>'Manage Domains'])
@endsection
            ...
            <div class="card-header">
                <div class="col-sm-10">
                    <ul class="nav nav-pills">
                        ...
                        <li class="nav-item">
                            <a class="nav-link  {{ $status == "trash" ? 'active' : '' }}" href="{{ url('/admin/domain?status=trash') }}">{{ __('Rejected') }} <span class="badge badge-danger">{{ $inactive }}</span></a>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="float-right mb-2">
                    <form type="get">
                        <div class="input-group form-row ">
                            <input type="text"  class="form-control" placeholder="Search ..." required="" name="src" autocomplete="off" value="{{ $request->src ?? '' }}">
                            <select  id="type_src" class="form-control" name="type">
                                <option value="domain">{{ __('Domain') }}</option>
                                <option value="tenant_id">{{ __('Profile Id') }}</option>
                            </select>
                ...
                <form method="post" action="{{ route('admin.domains.destroys') }}" class="basicform_with_reload">
                    ...
                    <div class="table-responsive">
                        ...
                        <tbody>
                            ...
                            <tr id="row{{ $row->id }}">
                                ...
                                <td><a href="{{ url(env('APP_URL_WITH_TENANT').$row->tenant_id) }}">{{ $row->tenant_id }}</a></td>
                                    <td>{{ $row->type == 0 ? env('APP_URL_WITH_TENANT').$row->tenant_id : $row->domain }}</td>
                                    <td>@if($row->status == 1)
                                ...
                                <td>
                                    @can('domain.edit')
                                    <a href="{{ route('admin.tenant.config',$row->tenant_id) }}"><i class="far fa-edit"></i> {{ __('Edit Domain') }}</a>
                                    @endcan
                                </td>
                            </tr>
                            @endforeach
                    ...
                    {{ $domains->appends($request->all())->links('vendor.pagination.bootstrap-4') }}
                </div>
            </form>
        </div>
    </div>
