...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>'Configure Domains','prev'=>
                    ...
                    <div class="card card-secondary">
                        <div class="card-header">
                            ...
                            <div class="card-header-action">
                                ...
                                <form class="d-none basicform" id="delete_form_{{ $info->subdomain->id }}"
                                    action="{{ route('admin.domains.destroys') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="method" value="delete">
                                    <input type="hidden" name="ids[]" value="{{ $info->subdomain->id }}">
                                </form>
                    ...
                    <div class="card card-secondary">
                        <div class="card-header">
                            ...
                            <div class="card-header-action">
                                ...
                                <form class="d-none basicform" id="delete_form_{{ $info->customdomain->id }}"
                                    action="{{ route('admin.domains.destroys') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="ids[]" value="{{ $info->customdomain->id }}">
                                     <input type="hidden" name="method" value="delete">
                                </form>
...
<div class="modal fade" id="subdomain" tabindex="-1" aria-labelledby="subdomain" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="basicform_with_reload" action="{{ route('admin.tenant.store') }}">
                @csrf
...
<div class="modal fade" id="subdomain" tabindex="-1" aria-labelledby="subdomain" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="basicform_with_reload" action="{{ route('admin.tenant.subdomain.update',$info->subdomain->id) }}">
                @csrf
                   <input type="hidden" name="tenant_id" value="{{ $info->id }}">
                ...
                <div class="form-group">
                    <label>{{ __('Add new subdomain') }}</label>
                    <div class="form-group">
                        <input type="text" class="form-control" name="subdomain" placeholder="enter subdomain" value="{{ $info->subdomain->domain }}">
                        <small class="form-text">{{ __('Example:') }} {example}.{{ env('APP_PROTOCOLESS_URL') }}</small>
                        </div>
                    </div>
...
<div class="modal fade" tabindex="-1" id="createModal" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="POST" class="basicform_with_reload" accept-charset="UTF-8" action="{{ route('admin.tenant.store') }}">
                @csrf
                 <input type="hidden" name="tenant_id" value="{{ $info->id }}">
                <input type="hidden" name="domain_type" value="2">
...
<div class="modal fade" tabindex="-1" id="createModal" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="POST" class="basicform_with_reload" accept-charset="UTF-8" action="{{ route('admin.domain.update',$info->customdomain->id) }}">
                @csrf
                @method('PUT')
                 <input type="hidden" name="tenant_id" value="{{ $info->id }}">
                <input type="hidden" name="domain_type" value="2">
...
@push('js')
<script src="{{ asset('backend/admin/assets/js/tenant.config.js') }}"></script>
@endpush