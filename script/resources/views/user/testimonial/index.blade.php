...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>'Manage Testimonials','button_name'=>'Create New','button_link'=> route('user.testimonial.create')])
@endsection
            ...
            <div class="card-body">
                <div class="table-responsive p-0">
                    <table class="table align-items-center mb-0">
                        ...
                        <tbody>
                            @php
                            $i = ($testimonial->currentpage()-1)* $testimonial->perpage() + 1
                            @endphp
                            @foreach ($testimonial as $item)
                            @php
                            $data = json_decode($item->testimonial_meta->value);
                            @endphp
                            <tr>
                                <td>
                                ...
                                <td>
                                    <p class="text-xs font-weight-bold mb-0">{{ Str::limit($item->name,30) }}</p>
                                </td>
                                <td>
                                ...
                                <td>
                                    <a class="btn btn-primary text-light px-3 mb-0"
                                        href="{{ route('user.testimonial.edit',$item->id) }}"><i
                                    ...
                                    <form class="d-none" id="delete_form_{{ $item->id }}"
                                        action="{{ route('user.testimonial.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
