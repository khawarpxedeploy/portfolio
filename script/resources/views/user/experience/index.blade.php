...
@section('head')
@include('layouts.backend.partials.headersection',['title'=>'All Experiences','button_name'=>'Create New','button_link'=> route('user.experience.create')])
@endsection
            ...
            <div class="card-body">
                <div class="table-responsive p-0">
                    <table class="table align-items-center mb-0">
                        ...
                        <tbody>
                            @php
                            $i = ($experience->currentpage()-1)* $experience->perpage() + 1
                            @endphp
                            @foreach ($experience as $item)
                            @php
                            $data = json_decode($item->experience_meta->value);
                            @endphp
                            <tr>
                                <td>
                                ...
                                <td>
                                    <a class="btn btn-primary text-light px-3 mb-0"
                                        href="{{ route('user.experience.edit',$item->id) }}"><i
                                    ...
                                    <form class="d-none" id="delete_form_{{ $item->id }}"
                                        action="{{ route('user.experience.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
                    ...
                    {{ $experience->links('vendor.pagination.bootstrap-4') }}
                </div>
            </div>
        </div>
    </div>
